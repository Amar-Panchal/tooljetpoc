...
export function GeneralSettings({ settings, updateData, instanceSettings }) {
  ...
  const [enableSignUp, setEnableSignUp] = useState(
    settings?.enable_sign_up || false
  );
  const [inheritSSO, setInheritSSO] = useState(
    settings?.inherit_s_s_o || false
  );
    ...
    <div className="card">
      <div className="card-header">
        <div className="card-title" data-cy="card-title">
          {t(
            "header.organization.menus.manageSSO.generalSettings.title",
            "General Settings"
          )}
        </div>
      </div>
      <div className="card-body">
        <form noValidate>
          <div className="form-group mb-3">
            <label className="form-check form-switch">
              <input
                ...
                type="checkbox"
                onChange={() =>
                  setEnableSignUp((enableSignUp) => !enableSignUp)
                }
              ...
              <span className="form-check-label" data-cy="enable-sign-up-label">
                {t(
                  "header.organization.menus.manageSSO.generalSettings.enableSignup",
                  "Enable signup"
                )}
              </span>
            </label>
          ...
          {!isSingleOrganization &&
            (instanceSettings.google.enabled ||
              instanceSettings.git.enabled) && (
              <div className="form-group mb-3">
                <label className="form-check form-switch">
                  <input
                    className="form-check-input"
                  ...
                  >
                    {t(
                      "header.organization.menus.manageSSO.generalSettings.allowDefaultSso",
                      `Allow default SSO`
                    )}
                  </span>
                </label>
                <div
                  className="d-flex tick-cross-info mb-2"
          ...
          <div className="form-group mb-3">
            <label className="form-label" data-cy="allowed-domains-label">
              {t(
                "header.organization.menus.manageSSO.generalSettings.allowedDomains",
                `Allowed domains`
              )}
            </label>
            <div>
              <input
                ...
                placeholder={t(
                  "header.organization.menus.manageSSO.generalSettings.enterDomains",
                  `Enter Domains`
                )}
                name="domain"
                value={domain}
          ...
          {!isSingleOrganization && (
            <div className="form-group mb-3">
              <label className="form-label" data-cy="workspace-login-url-label">
                {t(
                  "header.organization.menus.manageSSO.generalSettings.loginUrl",
                  `Login URL`
                )}
              </label>

              <div className="d-flex justify-content-between form-control">
                <p id="login-url" data-cy="workspace-login-url"></p>
                <img
