...
export default function ExportAppModal({
  ...
  return (
    <BootstrapModal
      ...
      contentClassName={`home-modal-component ${
    ...
    >
      <BootstrapModal.Header className="border-bottom">
        <BootstrapModal.Title
          data-cy={`${title.toLowerCase().replace(/\s+/g, "-")}-title`}
        >
          {title}
        </BootstrapModal.Title>
        <button
      ...
      {Array.isArray(versions) ? (
        <>
          <BootstrapModal.Body>
            <div className="py-2">
              <div
                className="current-version py-2"
                data-cy="current-version-section"
              >
              ...
              {versions.length >= 2 ? (
                ...
                >
                  <span
                    ...
                    data-cy="no-other-versions-found-text"
                  >
...
function InputRadioField({
  ...
  return (
    <span
      ...
      data-cy={`${String(versionName)
        .toLowerCase()
        .replace(/\s+/g, "-")}-version-wrapper`}
    >
      <input
        type="radio"
        value={versionId}
      ...
      >
        ...
        <span
          ...
          data-cy="created-date-label"
        >{`Created on ${moment(versionCreatedAt).format("Do MMM YYYY")}`}</span>
      </label>
    </span>
  );
}
