...
class ManageGroupPermissionResourcesComponent extends React.Component {
  ...
  componentDidMount() {
    if (this.props.groupPermissionId)
  ...
  componentDidUpdate(prevProps) {
    if (
      ...
      this.props.groupPermissionId !== prevProps.groupPermissionId
    ) {
      this.fetchGroupAndResources(this.props.groupPermissionId);
    }
  }
  ...
  updateAppGroupPermission = (app, groupPermissionId, action) => {
    ...
    if (action === "hideFromDashboard") {
      actionParams["hideFromDashboard"] = !this.canAppGroupPermission(
        app,
        groupPermissionId,
        "hideFromDashboard"
      );
  ...
  canAppGroupPermission = (app, groupPermissionId, action) => {
    let appGroupPermission = this.findAppGroupPermission(
      app,
      groupPermissionId
    );
    switch (action) {
      ...
      case "hideFromDashboard":
        return (
          appGroupPermission?.read && appGroupPermission?.hide_from_dashboard
        );
      default:
        return false;
    }
  ...
  findAppGroupPermission = (app, groupPermissionId) => {
    return app.app_group_permissions.find(
      (permission) => permission.group_permission_id === groupPermissionId
    );
  };
  ...
  removeAppFromGroup = (groupPermissionId, appId, appName) => {
    if (
      window.confirm(
        `Are you sure you want to delete this app - ${appName}?`
      ) === false
    )
      return;
    const updateParams = {
    ...
    return selected?.map((d) => {
      return (
        <div className="selected-item tj-ms" key={d.value}>
          <FilterPreview
            text={d.name}
            onClose={() => this.removeSelection(selected, d.value)}
          />
        </div>
      );
    });
  ...
  render() {
    ...
    const folder_permission = groupPermission
      ? groupPermission.folder_create &&
        groupPermission.folder_delete &&
        groupPermission.folder_update
      : false;
          ...
          {isLoadingGroup ? (
            ...
            <div className="card">
              <nav className="nav nav-tabs">
                ...
                >
                  {this.props.t(
                    "header.organization.menus.manageGroups.permissionResources.users",
                    "Users"
                  )}
                </a>
                <a
              ...
              <div className="card-body">
                <div className="tab-content">
                        ...
                        <div className="col-1">
                          <div
                            className={`btn btn-primary w-100 ${
                              isAddingApps ? "btn-loading" : ""
                            } ${selectedAppIds.length === 0 ? "disabled" : ""}`}
                            onClick={() =>
                              this.addSelectedAppsToGroup(groupPermission.id)
                            }
                    ...
                    {groupPermission?.group !== "all_users" && (
                      <div className="row">
                        ...
                        <div className="col-auto">
                          <div
                            className={`btn btn-primary w-100 ${
                              isAddingUsers ? "btn-loading" : ""
                            } ${selectedUsers.length === 0 ? "disabled" : ""}`}
                            onClick={() =>
                              this.addSelectedUsersToGroup(
                                groupPermission.id,
                                selectedUsers
                              )
                            }
                  ...
                  <div
                    className={`tab-pane ${
                      currentTab === "permissions" ? "active show" : ""
                    }`}
                  >
                    <div>
                      <div className="table-responsive">
                                  ...
                                  <td className="text-muted">
                                    <div>
                                      <label className="form-check form-check-inline">
                                        ...
                                        <span
                                          className="form-check-label"
                                          data-cy="workspace-variable-create-label"
                                        >
                                          {this.props.t(
                                            "header.organization.menus.manageGroups.permissionResources.createUpdateDelete",
                                            "Create/Update/Delete"
...
export const ManageGroupPermissionResources = withTranslation()(
