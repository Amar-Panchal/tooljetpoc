...
async function executeRunPycode(
  _ref,
  code,
  query,
  editorState,
  ...
  const evaluatePythonCode = async (pyodide) => {
    ...
    try {
      ...
      const actions = generateAppActions(
        _ref,
        query.id,
        mode,
        editorState,
      ...
      const errorType = err.message.includes("SyntaxError")
        ? "SyntaxError"
        : "NameError";
...
export async function runPythonTransformation(
  currentState,
  rawData,
  transformation,
  query,
...
export async function runTransformation(
  ...
  if (transformationLanguage === "python") {
    result = await runPythonTransformation(
      currentState,
      data,
      transformation,
      query,
  ...
  if (transformationLanguage === "javascript") {
    try {
      ...
      const $errorMessage = _.has(ERROR_TYPES, $error)
        ? `${$error} : ${err.message}`
        : err || "Unknown error";
      if (mode === "edit") toast.error($errorMessage);
      result = {
...
export async function executeActionsForEventId(
  _ref,
  eventId,
  component,
  mode,
...
export function onQueryConfirmOrCancel(
  _ref,
...
function showModal(_ref, modal, show) {
  ...
  const modalMeta =
    _ref.state.appDefinition.pages[_ref.state.currentPageId].components[
      modalId
    ];
  const newState = {
    currentState: {
  ...
  if (event) {
    switch (event.actionId) {
      ...
      case "go-to-app": {
        ...
        const queryParams = event.queryParams?.reduce(
          (result, queryParam) => ({
            ...result,
            ...{
              [resolveReferences(queryParam[0], _ref.state.currentState)]:
                resolveReferences(
                  queryParam[1],
                  _ref.state.currentState,
                  undefined,
        ...
        if (mode === "view") {
          ...
          if (
            confirm(
              "The app will be opened in a new tab as the action is triggered from the editor."
            )
          ) {
      ...
      case "unset-page-variable": {
        ...
        const customPageVariables = _.omit(
          _ref.state.currentState.page.variables,
          key
        );
      ...
      case "control-component": {
        const component = Object.values(
          _ref.state.currentState?.components ?? {}
        ).filter((component) => component.id === event.componentId)[0];
        const action = component[event.componentSpecificActionHandle];
        const actionArguments = _.map(
          event.componentSpecificActionParams,
          (param) => ({
            ...param,
        ...
        const actionPromise = action(
          ...actionArguments.map((argument) => argument.value)
        );
...
export async function onEvent(_ref, eventName, options, mode = "edit") {
  ...
  if (eventName === "onPageLoad") {
    await executeActionsForEventId(
      _ref,
      "onPageLoad",
      { definition: { events: [options] } },
      mode,
      customVariables
    );
  }
...
export function previewQuery(
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (query.kind === "runjs") {
      ...
      queryExecutionPromise = executeRunPycode(
        _ref,
        query.options.code,
        query,
        editorState,
...
export function runQuery(
  ...
  const query = _ref.state.app.data_queries.find(
  ...
  if (dataQuery.options.requestConfirmation) {
    const queryConfirmationList = _ref.state?.queryConfirmationList
        ...
        .then(async (data) => {
          ...
          if (dataQuery.options.showSuccessNotification) {
            const notificationDuration =
              dataQuery.options.notificationDuration * 1000 || 5000;
...
export function setTablePageIndex(_ref, tableId, index) {
  ...
  const table = Object.entries(_ref.state.currentState.components).filter(
