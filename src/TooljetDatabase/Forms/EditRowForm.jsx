...
const EditRowForm = ({ onEdit, onClose }) => {
  const { organizationId, selectedTable, columns, selectedTableData } =
  ...
  const handleChange = (columnName, value) => {
    ...
    const shouldUpdate =
      _.get(_rowData, columnName) !== _.get(rowData, columnName);
  ...
  const primaryColumn =
    columns.find((column) => column.isPrimaryKey)?.accessor || null;
      ...
      <div className="card-body">
        <div>
          ...
          {selectedRow &&
            ...
            columns?.map(
              (
                { Header, accessor, dataType, isPrimaryKey, column_default },
                index
              ) => {
                const currentValue = selectedTableData.find(
...
const RenderElement = ({
  ...
  const placeholder =
    defaultValue?.length > 0 ? removeQuotes(defaultValue.split("::")[0]) : "";
