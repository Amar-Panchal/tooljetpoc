...
const CreateColumnDrawer = () => {
  const { organizationId, selectedTable, setColumns, setSelectedTableData } =
    useContext(TooljetDatabaseContext);
  const [isCreateColumnDrawerOpen, setIsCreateColumnDrawerOpen] =
    useState(false);
        ...
        <CreateColumnForm
          onCreate={() => {
            if (data?.result?.length > 0) {
              setColumns(
                data?.result.map(
                  ({ column_name, data_type, keytype, ...rest }) => ({
                    Header: column_name,
