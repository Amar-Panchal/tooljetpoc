...
class DataSourceManagerComponent extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.selectedDataSource !== this.props.selectedDataSource) {
      let dataSourceMeta = this.getDataSourceMeta(
      ...
      this.setState({
        ...
        dataSourceMeta,
        dataSourceSchema:
          this.props.selectedDataSource?.plugin?.manifestFile?.data,
        selectedDataSourceIcon:
          this.props.selectedDataSource?.plugin?.iconFile?.data,
      });
    }
  }
  ...
  getDataSourceMeta = (dataSource) => {
    ...
    if (dataSource?.pluginId) {
      let dataSourceMeta = camelizeKeys(
        dataSource?.plugin?.manifestFile?.data.source
      );
  ...
  createDataSource = () => {
    const { appId, options, selectedDataSource, selectedDataSourcePluginId } =
      this.state;
    ...
    if (name.trim() !== "") {
      if (selectedDataSource.id) {
        ...
        datasourceService
          .save(selectedDataSource.id, appId, name, parsedOptions)
          .then(() => {
            this.setState({ isSaving: false });
            this.hideModal();
            toast.success(
              this.props.t(
                "editor.queryManager.dataSourceManager.toast.success.dataSourceSaved",
                "Datasource Saved"
              ),
              { position: "top-center" }
            );
        ...
        this.setState({ isSaving: true });
        datasourceService
          .create(appId, appVersionId, pluginId, name, kind, parsedOptions)
          .then(() => {
            this.setState({ isSaving: false });
            this.hideModal();
            toast.success(
              this.props.t(
                "editor.queryManager.dataSourceManager.toast.success.dataSourceAdded",
                "Datasource Added"
              ),
              { position: "top-center" }
            );
  ...
  handleSearch = (searchQuery, activeDatasourceList) => {
    ...
    const filteredDatasources = this.datasourcesGroups().filter(
      (group) => group.key === activeDatasourceList
    )[0].list;
            ...
            <div className="selected-datasource-list-content">
              <Tab.Content>
                {suggestingDatasources ? (
                  <div className="suggestion-container">
                    <h4 className="justify-content-start">
                      {this.props.t(
                        "editor.queryManager.dataSourceManager.suggestDataSource",
                        "Suggest Datasource"
                      )}
                    </h4>
                  ...
                  <>
                    ...
                    {datasources.map((datasource) => (
                      <Tab.Pane
                        transition={false}
                        active={
                          this.state.activeDatasourceList === datasource.key
                        }
                ...
                {!suggestingDatasources &&
                  ...
                  this.state.filteredDatasources.length === 0 && (
        ...
        <div className="datasource-modal-sidebar-footer">
          <p>
            ...
            <span className="link-span" onClick={updateSuggestionState}>
              {this.props.t(
                "editor.queryManager.dataSourceManager.suggest",
                "Suggest"
              )}
            </span>
          </p>
  ...
  renderCardGroup = (source, type) => {
    const renderSelectedDatasource = (dataSource) =>
      this.selectDataSource(dataSource);

    if (this.state.queryString && this.state.queryString.length > 0) {
      const filteredDatasources = this.state.filteredDatasources.map(
        (datasource) => {
      ...
      <div>
        <Modal
          ...
          className={
            selectedDataSource
              ? "animation-fade"
              : "select-datasource-list-modal animation-fade"
          }
        ...
        >
          <Modal.Header className="justify-content-start">
            {selectedDataSource && (
              <div
                ...
                role="button"
                onClick={() =>
                  this.setState({ selectedDataSource: false }, () =>
                    this.onExit()
                  )
                }
              >
            ...
            <Modal.Title>
              ...
              {!selectedDataSource && (
                <span className="text-muted" data-cy="title-add-new-datasource">
                  {this.props.t(
                    "editor.queryManager.dataSourceManager.addNewDataSource",
                    "Add new datasource"
                  )}
                </span>
              )}
          ...
          <Modal.Body>
            ...
            {!selectedDataSource &&
              this.segregateDataSources(
                this.state.suggestingDatasources,
                this.props.darkMode
              )}
          </Modal.Body>
                      ...
                      {isCopied ? (
                        <center className="my-2">
                          <span className="copied" data-cy="label-ip-copied">
                            {this.props.t(
                              "editor.queryManager.dataSourceManager.copied",
                              "Copied"
                            )}
                          </span>
                        </center>
                        ...
                        >
                          <button
                            ...
                            className={`copy-button ${
                              this.props.darkMode && "dark-button"
                            }`}
                          ...
                          >
                            ...
                            {this.props.t(
                              "editor.queryManager.dataSourceManager.copy",
                              "Copy"
                            )}
                          </button>
                        </CopyToClipboard>
              ...
              <div className="col-auto" data-cy="button-test-connection">
                <TestConnection
                  ...
                  pluginId={
                    ...
                    this.state.selectedDataSourcePluginId
                  }
                  options={options}
              ...
              <Modal.Footer>
                ...
                <div className="col-auto">
                  ...
                  >
                    {isSaving
                      ? this.props.t(
                          "editor.queryManager.dataSourceManager.saving" +
                            "...",
                          "Saving..."
                        )
                      : this.props.t("globals.save", "Save")}
    ...
    <div className="empty">
      ...
      <center
        className={`empty-results ${
          suggestionUI ? "suggestionUI-results" : ""
        }`}
      >
        <img
        ...
        {status ? (
          <div>
            <p className="text-success mt-2">
              {t(
                "editor.queryManager.dataSourceManager.noteTaken",
            ...
            >
              {t(
                "editor.queryManager.dataSourceManager.goToAllDatasources",
