...
import { source } from "./QueryEditors";
...
class QueryManagerComponent extends React.Component {
  ...
  setStateFromProps = (props) => {
    ...
    const selectedDataSource =
      paneHeightChanged || queryPaneDragged
    ...
    const paneHeightChanged =
      this.state.queryPaneHeight !== props.queryPaneHeight;
    const dataQueries = props.dataQueries?.length
      ? props.dataQueries
      : this.state.dataQueries;
    const queryPaneDragged =
      this.state.isQueryPaneDragging !== props.isQueryPaneDragging;
    ...
    this.setState(
      {
        ...
        isSourceSelected:
          paneHeightChanged || queryPaneDragged
            ...
            : props.isSourceSelected,
        selectedDataSource: this.skipSettingSourceToNull.current
          ? this.state.selectedDataSource
          : selectedDataSource,
        queryPreviewData:
          this.state.selectedQuery?.id !== props.selectedQuery?.id
            ? undefined
            : props.queryPreviewData,
        selectedQuery:
          props.mode === "create" ? selectedQuery : this.state.selectedQuery,
        ...
        shouldRunQuery:
          props.mode === "edit"
            ...
            : this.props.isSourceSelected,
      },
      () => {
        let source = props.dataSources.find(
        ...
        if (this.skipSettingSourceToNull.current) {
          ...
          this.setState({ selectedDataSource: null }, () => {
            this.setState({
              selectedDataSource:
                props.mode === "edit" ? source : selectedDataSource,
            });
          });
        }
      }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!isEmpty(this.state.updatedQuery)) {
      const query = nextProps.dataQueries.find(
        (q) => q.id === this.state.updatedQuery.id
      );
      if (query) {
        ...
        const prevLoading = this.state.currentState?.queries[query.name]
          ...
          : false;
        if (
          !isEmpty(nextProps.selectedQuery) &&
          !isEqual(this.state.selectedQuery, nextProps.selectedQuery)
        ) {
    ...
    if (
      ...
      (Object.keys(diffProps).length === 1 &&
        "addNewQueryAndDeselectSelectedQuery" in diffProps) ||
      (!this.props.isUnsavedQueriesAvailable &&
        nextProps.isUnsavedQueriesAvailable)
    ) {
      return;
    }
  ...
  validateQueryName = () => {
    ...
    if (mode === "create") {
      return (
        dataQueries.find((query) => query.name === queryName) === undefined &&
    ...
    if (existingQuery) {
      return (
        existingQuery.id === selectedQuery.id && queryNameRegex.test(queryName)
      );
    }
  ...
  computeQueryName = (kind) => {
    const { dataQueries } = this.props;
    const currentQueriesForKind = dataQueries.filter(
  ...
  createOrUpdateDataQuery = () => {
    ...
    const dataSourceId =
      selectedDataSource.id === "null" ? null : selectedDataSource.id;
    const pluginId =
      selectedDataSource.pluginId || selectedDataSource.plugin_id;

    const isQueryNameValid = this.validateQueryName();
    if (!isQueryNameValid) {
      toast.error(
        "Invalid query name. Should be unique and only include letters, numbers and underscore."
      );
      return;
    }
  ...
  cleanFocusedFields = (newOptions) => {
    ...
    Object.keys(diffFields).forEach((key) => {
      if (
        newOptions[key] === "" &&
        this.defaultOptions.current[key] === undefined
      ) {
  ...
  validateNewOptions = (newOptions) => {
    ...
    if (this.state.selectedQuery) {
      ...
      if (isQueryChanged) {
        ...
        if (
          Object.is(updatedOptions.body_toggle, !this.state.options.body_toggle)
        ) {
  ...
  toggleOption = (option) => {
    const currentValue = this.state.options[option]
  ...
  executeQueryNameUpdation = (newName) => {
    if (this.state.queryName === newName)
      return this.setState({ renameQuery: false });
    const isNewQueryNameAlreadyExists = this.state.dataQueries.some(
  ...
  render() {
    ...
    if (selectedDataSource) {
      const sourcecomponentName =
        selectedDataSource.kind.charAt(0).toUpperCase() +
        selectedDataSource.kind.slice(1);
      ElementToRender = selectedDataSource?.pluginId ? source : "";
    }
    ...
    return (
      <div
        className={cx(
          `query-manager ${this.props.darkMode ? "theme-dark" : ""}`,
          {
      ...
      >
        ...
        <div className="row header" style={{ padding: "8px 0" }}>
          <div
            className="col d-flex align-items-center px-3 h-100 font-weight-500 py-1"
            style={{ gap: "10px" }}
          >
            {(addingQuery || editingQuery) && selectedDataSource && (
              <>
                ...
                <div className="query-name-breadcrum d-flex align-items-center">
                  ...
                  >
                    {this.state.renameQuery ? (
                      <input
                        ...
                        onBlur={({ target }) =>
                          this.executeQueryNameUpdation(target.value)
                        }
                      />
                    ) : (
                      queryName
                  ...
                  <span
                    className={`breadcrum-rename-query-icon ${
                      this.state.renameQuery && "d-none"
                    }`}
              ...
              <button
                onClick={() => {
                  if (this.state.isFieldsChanged || this.state.addingQuery) {
                    this.setState({ shouldRunQuery: true }, () =>
                ...
                className={`border-0 default-secondary-button float-right1 ${
                  this.props.darkMode ? "theme-dark" : ""
                } ${this.state.selectedDataSource ? "" : "disabled"} ${
              ...
              >
                <span
                  className={`query-manager-btn-svg-wrapper d-flex align-item-center query-icon-wrapper query-run-svg ${
                    this.state.currentState.queries[selectedQuery.name]
                      ?.isLoading && "invisible"
                  }`}
                >
                  <svg
          ...
          <div>
                ...
                !this.state.isSourceSelected && (
                  <div className="datasource-picker">
                    {!this.state.isSourceSelected && (
                      ...
                      >
                        {this.props.t(
                          "editor.queryManager.selectDatasource",
                          "Select Datasource"
                        )}
                      </label>
                    )}{" "}
              ...
              {selectedDataSource && (
                <div style={{ padding: "0 32px" }}>
                  <div>
                    <ElementToRender
                      pluginSchema={
                        this.state.selectedDataSource?.plugin?.operationsFile
                          ?.data
                      }
            ...
            {selectedDataSource && (addingQuery || editingQuery) && (
              <div className="advanced-options-container font-weight-400 border-top query-manager-border-color">
                <div className="advance-options-input-form-container">
                  ...
                  <div className=" mx-4">
                    <CustomToggleSwitch
                      ...
                      darkMode={this.props.darkMode}
                      label={this.props.t(
                        "editor.queryManager.notificationOnSuccess",
                  ...
                  {this.state.options.showSuccessNotification && (
                    <div
                      className="mx-4"
                      style={{ paddingLeft: "100px", paddingTop: "12px" }}
                    >
                      <div className="row mt-1">
                        <div className="col-auto" style={{ width: "200px" }}>
                          <label
                            className="form-label p-2 font-size-12"
                            data-cy={"label-success-message-input"}
                          >
                            {this.props.t(
                              "editor.queryManager.successMessage",
                              "Success Message"
                            )}
                          </label>
                        </div>
                        <div className="col">
                          <CodeHinter
                            ...
                            onChange={(value) =>
                              this.optionchanged("successMessage", value)
                            }
                      ...
                      <div className="row mt-3">
                        <div className="col-auto" style={{ width: "200px" }}>
                          ...
                          >
                            {this.props.t(
                              "editor.queryManager.notificationDuration",
                ...
                >
                  {this.props.t(
                    "editor.queryManager.eventsHandler",
                    "Events Handler"
                  )}
                </div>
                  ...
                  <EventManager
                    ...
                    pages={
                      this.props.appDefinition?.pages
                        ? Object.entries(this.props.appDefinition?.pages).map(
                            ([id, page]) => ({ ...page, id })
                          )
                        : []
                    }
...
export const QueryManager = withTranslation()(
  React.memo(QueryManagerComponent)
);
